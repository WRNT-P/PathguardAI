...
> 🕗 **เขียน 22 ส.ค. 2026 · ยังไม่ครอบสิ่งที่สร้างวันที่ 26** ตรวจกับโค้ดเมื่อ 27 ส.ค.
> เอกสารนี้**ไม่มี**: `POST /api/sos` · การจับคู่เครื่อง (`pairing_codes`, `POST /api/pair`) ·
> การขออนุมัติเดินทาง C-3 (`trip_requests`) · `users.severity_level` และผลของมันต่อรัศมี
> ค้นหากับจำนวนรายการแนะนำ · `routine_patterns` และปัจจัยช่วงเวลาของ Module 5 ·
> `PUT /api/patients/{id}/places/home`
> **ของที่เขียนไว้ยังถูก ที่หายคือของใหม่** สายที่ครบกว่าอยู่ที่ `backend/API_CONTRACT_APP.md`
> และ `backend/REPORT_VS_CODE.md`
