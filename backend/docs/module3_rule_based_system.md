...
> 🕗 **เขียน 22 ส.ค. 2026 · เกณฑ์เพิ่มมาหนึ่งตัวหลังจากนั้น** ตรวจกับ
> `rule_repository.KNOWN_THRESHOLDS` เมื่อ 27 ส.ค. — ตอนนี้มี 7 ตัว เอกสารนี้ไม่มี
> **`sos_cooldown_seconds`** (60 วิ เพิ่มพร้อมปุ่ม SOS เมื่อ 26 ส.ค.)
> **สำคัญ:** `get_all_thresholds` จะไม่ยอมโหลดถ้าเกณฑ์ที่รู้จักขาดไปแม้ตัวเดียว
> ดังนั้นบนฐานข้อมูลที่ยังไม่ seed ใหม่ อันนี้ทำให้ **การคำนวณคะแนนเสี่ยงล้มทั้งระบบ**
> ไม่ใช่แค่ SOS — รัน `python -m app.mock.seed_risk_rules` ก่อน deploy
> ส่วนน้ำหนักกับแหล่งอ้างอิงทางการแพทย์ในเอกสารนี้ยังตรงกับโค้ด
